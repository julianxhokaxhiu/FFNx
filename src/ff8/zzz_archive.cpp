...
bool Zzz::openHeader()
{
	...
	for (uint64_t i = 0; i < fileCount; ++i)
	{
		...
		if (remainingSize < ZZZ_FILENAME_MAX_SIZE + 16) {
			// Read data again
			if (remainingSize > 0) {
		...
		cur += sizeof(uint32_t);
		if (fileNameSize > ZZZ_FILENAME_MAX_SIZE) {
			ffnx_error("Zzz::%s: cannot read toc entry %d, fileNameSize > %d\n", __func__, i, ZZZ_FILENAME_MAX_SIZE);
