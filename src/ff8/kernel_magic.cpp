...
// Only the call site is hooked, so the dispatcher itself is untouched and can be
// called as is. That one site is enough: the queued player commands all run
// through BattleAction_ExecuteCommand, and its other eleven callers pass a fixed
// command that is never a stocked draw.
