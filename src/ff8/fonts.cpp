...
void font_with_font8c_sub_4A1CF0(ff8_draw_menu_sprite_texture_infos_short *texture_infos, ff8_font *fonts)
{
    ...
    ffnx_info("%s: type=%d color=%X texture_infos->x=%lf flt_2231088=%f viewport=(%f %f) flt_2231090=%f x=%f y=%f w=%f h=%f z=%f\n", __func__,
        type, color_related, double(texture_infos->x), flt_2231088, offset_menu_viewport_and_stuff_off_D8A428[4], offset_menu_viewport_and_stuff_off_D8A428[6],
        flt_2231090, x2, y2, width, height, z_related);
...
void fonts_with_font8c_1_sub_49D190(ff8_draw_menu_sprite_texture_infos_short *texture_infos, ff8_font *fonts)
{
    ...
    *((uint16_t *)&(texture_infos->u) + 1) += kernel_sysfont[*pointer_to_iterate_to++].field_1;

    texture_infos->palID = ((texture_infos->palID - 0x3812) << 1) + 0x3812;
...
{
    ffnx_trace("%s: texID=%X color=%X pos=(%d, %d) uv=(%d, %d) palID=%X size=(%d, %d)\n", __func__, texture_infos->texID, texture_infos->color, texture_infos->x, texture_infos->y, texture_infos->u, texture_infos->v, texture_infos->palID, texture_infos->w, texture_infos->h);
...
void fonts_field_with_font8c_sub_49C3D0(ff8_draw_menu_sprite_texture_infos_short *texture_infos, ff8_font *fonts)
{
    ffnx_trace("%s\n", __func__);
    ...
    *(uint16_t *)&texture_infos->u = 12 * (((character >> 1) % 21) | (((character >> 1) / 21) << 8));
    ...
    if (fonts_fieldtdw_odd->graphics_object48 != nullptr && fonts_fieldtdw_even->graphics_object48 != nullptr) {
        int v4 = (texture_infos->palID >> 6) - fonts_fieldtdw_even->field_40;
        texture_infos->palID = ((texture_infos->palID >> 6) << 6) | ((fonts_fieldtdw_even->field_3E >> 4) & 0x3F);

        fonts = (v4 & 1) != 0 ? fonts_fieldtdw_odd : fonts_fieldtdw_even;
    }

    return font_with_font8c_sub_4A1CF0(texture_infos, fonts);
}
