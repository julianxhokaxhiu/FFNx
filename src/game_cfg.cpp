...
void ff8_set_game_paths(int install_options, char *_app_path, const char *_dataDrive)
{
	...
	if (!steam_edition && !ff8_remastered_edition && !data_drive.empty())
	{
...
{
	int ret = ff8_remastered_edition ? 0 : ff8_externals.reg_get_midiguid((LPBYTE)midi_guid);
	LPDWORD default_midi_guid[4] = {0, 0, 0, 0};

	if (ff8_remastered_edition || memcmp(midi_guid, default_midi_guid, 16) == 0) {
...
{
	uint32_t ret = ff8_remastered_edition ? 0x00100021 : ff8_externals.reg_get_graphics();
