...
static ff7_graphics_object* prompt_graphics_object = nullptr;
static ff7_texture_set* prompt_original_texture_set = nullptr;
static ff7_texture_set* prompt_polygon_original_texture_set = nullptr;
static uint32_t prompt_texture = 0;
static uint32_t prompt_texture_width = 0;
static uint32_t prompt_texture_height = 0;
...
void universal_buttons_unload()
{
  ...
  ff7_polygon_set* polygon_set = prompt_graphics_object
    ? (ff7_polygon_set*)prompt_graphics_object->polygon_set
    : nullptr;
  if (prompt_graphics_object && prompt_graphics_object->hundred_data
      && prompt_original_texture_set)
  {
    private_texture_set = (ff7_texture_set*)prompt_graphics_object->hundred_data->texture_set;
    prompt_graphics_object->hundred_data->texture_set = (struct texture_set*)prompt_original_texture_set;
  }
  if (polygon_set && polygon_set->hundred_data && prompt_polygon_original_texture_set)
    polygon_set->hundred_data->texture_set = (struct texture_set*)prompt_polygon_original_texture_set;

  ff7_externals.sub_671082(&prompt_graphics_object);
  if (private_texture_set && private_texture_set != prompt_original_texture_set)
  {
    delete private_texture_set->ogl.gl_set;
    delete[] private_texture_set->texturehandle;
    delete private_texture_set;
  }
  prompt_original_texture_set = nullptr;
  prompt_polygon_original_texture_set = nullptr;
  if (prompt_texture)
    newRenderer.deleteTexture(prompt_texture);
  prompt_texture = 0;
  prompt_texture_width = 0;
  prompt_texture_height = 0;
}
...
void universal_buttons_load(struc_3* graphics_context, char* template_path, ff7_game_obj* game_object)
{
  ...
  const char* atlas_name = current_prompt_atlas == prompt_atlas::keyboard
    ? "buttons_pc_en.png"
    : current_prompt_atlas == prompt_atlas::xbox ? "buttons.png" : "buttons_ps4.png";
  _snprintf(path, sizeof(path), R"(%s\data\png\%s)", basedir, atlas_name);
  if (current_prompt_atlas != prompt_atlas::playstation && !fileExists(path))
  {
  ...
  prompt_graphics_object = ff7_externals.engine_load_graphics_object_6710AC(
    1, 12, graphics_context, template_path, (int)game_object->dx_sfx_something);
  if (!prompt_graphics_object || !prompt_graphics_object->hundred_data
      || !prompt_graphics_object->hundred_data->texture_set)
    return;

  prompt_texture = newRenderer.createTextureLibPng(
    path, &prompt_texture_width, &prompt_texture_height, true);
  uint32_t expected_width = current_prompt_atlas == prompt_atlas::keyboard ? 1000 : 512;
  uint32_t expected_height = current_prompt_atlas == prompt_atlas::keyboard ? 1200 : 512;
  if (!prompt_texture || prompt_texture_width != expected_width
      || prompt_texture_height != expected_height)
  {
  ...
  ff7_polygon_set* polygon_set = (ff7_polygon_set*)prompt_graphics_object->polygon_set;
  ...
  prompt_original_texture_set = (ff7_texture_set*)prompt_graphics_object->hundred_data->texture_set;
  prompt_polygon_original_texture_set = (ff7_texture_set*)polygon_set->hundred_data->texture_set;
  ff7_tex_header* texture_header = (ff7_tex_header*)prompt_polygon_original_texture_set->tex_header;
  uint32_t texture_count = std::max(std::max(texture_header->palettes,
    texture_header->palette_index + 1), 8u);
  ff7_texture_set* private_texture_set = new ff7_texture_set(*prompt_polygon_original_texture_set);
  private_texture_set->texturehandle = new uint32_t[texture_count];
  memcpy(private_texture_set->texturehandle, prompt_polygon_original_texture_set->texturehandle,
    texture_count * sizeof(*private_texture_set->texturehandle));
  private_texture_set->texturehandle[7] = prompt_texture;
  private_texture_set->ogl.gl_set = prompt_polygon_original_texture_set->ogl.gl_set
    ? new gl_texture_set(*prompt_polygon_original_texture_set->ogl.gl_set)
    : nullptr;
  if (private_texture_set->ogl.gl_set)
    private_texture_set->ogl.gl_set->force_filter = true;
  private_texture_set->ogl.external = true;
  private_texture_set->ogl.width = prompt_texture_width;
  private_texture_set->ogl.height = prompt_texture_height;
  prompt_graphics_object->hundred_data->texture_set = (struct texture_set*)private_texture_set;
...
{
  if (prompt_graphics_object && prompt_texture)
    ff7_externals.engine_draw_graphics_object_66E641(prompt_graphics_object, game_object);
}

void universal_buttons_reset()
{
  if (prompt_graphics_object)
    ff7_externals.reset_field_54_graphics_object_66E62C(prompt_graphics_object);
}
...
static bool sprite_for_button(int button, prompt_sprite* sprite)
{
  if (!prompt_graphics_object)
    return false;
  ...
  if (current_prompt_atlas == prompt_atlas::keyboard)
  {
    if (!ff7_externals.input_mapping || button < 0 || button >= 25)
      return false;
    int cell = keyboard_atlas_cell((byte)ff7_externals.input_mapping[button]);
    if (cell < 0)
      return false;
    *sprite = { prompt_graphics_object, cell % 10 * 100, cell / 10 * 100, 100, 100,
      (int)prompt_texture_width, (int)prompt_texture_height };
    return true;
  }
  ...
  *sprite = { prompt_graphics_object, column * 100, row * 100, 100, 100,
    (int)prompt_texture_width, (int)prompt_texture_height };
  return true;
}

static bool submit_field_quad(const prompt_sprite& sprite, float x, float y, float z)
{
...
int universal_buttons_draw_field_prompt(int button, int x, int y, float z)
{
  ...
  if (!sprite_for_button(button, &sprite))
    return x;

  if (!submit_field_quad(sprite, (float)x, (float)y - prompt_size / 4.0f, z))
  {
